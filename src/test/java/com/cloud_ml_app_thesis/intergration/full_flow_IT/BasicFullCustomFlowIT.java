...
public class BasicFullCustomFlowIT {
    ...
    void shouldTrainCustomModel() throws IOException {
        ...
        File datasetFile = new ClassPathResource("datasets/train.csv").getFile();
        ...
        waitForTaskCompletion(taskId, "COMPLETED");

        trainingId = given()
                .port(port)
                .header("Authorization", "Bearer " + jwtToken)
                .when()
                .get("/api/tasks/{taskId}/training-id", taskId)
                .then()
                .statusCode(200)
                .extract()
                .jsonPath()
                .getInt("dataHeader.trainingId");

        Assertions.assertNotNull(trainingId, "❌ training is null after waiting for training.");
        System.out.println("Training ID after completion: " + trainingId);
    }
    ...
    void shouldCategorizeModel() {
        Assumptions.assumeTrue(trainingId != null, "Skipping test because trainingId is null");
        ...
        Response finalizedResponse = given()
                ...
                .post("/api/models/{trainingId}/model", trainingId)
                .then()
                .statusCode(200)
    ...
    void shouldPredictCustomModel() throws IOException {
        ...
        Response predictionResponse = given()
                ...
                .when()
                .post("/api/model-exec/execute-custom")
                .then()
                .statusCode(202)
        ...
        waitForTaskCompletion(predictionTaskId, "COMPLETED");
    ...
    private void waitForTaskCompletion(String taskId, String expectedStatus) {
        int maxTries = 15;
        ...
        for (int i = 1; i <= maxTries; i++) {
            ...
            if (expectedStatus.equalsIgnoreCase(status)) {
                System.out.printf("✅ Task %s reached status: %s%n", taskId, expectedStatus);
                return;
        ...
        fail("❌ Task " + taskId + " did not reach expected status: " + expectedStatus + " within timeout.");
    }
